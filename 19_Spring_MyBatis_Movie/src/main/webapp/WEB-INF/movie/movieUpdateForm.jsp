...
s_selbox[4] = new Array('뉴질랜드','오스트레일리아');
	...
	function init(f){
		...
		for(i=0; i < f.first.length; i++){
			if(f.first.options[i].value == "${mb.continent}"){ //일치 조건 
		...
		if(f.first.options[findex].value != ""){
			...
			for(i=0; i < f.second.length; i++){
				if(f.second.options[i].value == "${mb.nation}"){
...
<table border = "1" width = "60%" align = "center">
	<tr>
		<td>영화 제목 :</td>
		<td><input type = "text" name = "title" value = "${mb.title }">
	...
	<tr>
		...
		${grade }
		<input type = "radio" name = "grade" value = "${grade }" <c:if test = "${grade eq movieBean.grade }"> checked</c:if>>
		</c:forEach>
		<form:errors cssClass="err" path = "grade"/></td>
		</tr>
	<tr>
		<td>출연 배우 :</td> 
		<td><input type = "text" name = "actor" value = "${mb.actor }"></td>
	</tr>
	<tr>
